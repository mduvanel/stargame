...
public class BattleActivity extends Activity implements IDPadPeeker
{
    ...
    protected void onDestroy()
    {
        ...
        {
            Logger.e( String.format( "Caught exception %s when exiting Controller Thread",
                                     e.getMessage() ) );
        }
    ...
    public boolean onTouchEvent( MotionEvent oEvent, Orientation eOrientation )
    {
        ...
        switch ( oEvent.getAction() & MotionEvent.ACTION_MASK )
        {
            ...
            case MotionEvent.ACTION_POINTER_UP:
                ...
                if ( iPointerUp < 2 )
                {
                    double dNewDist = Math.pow( oEvent.getX( 0 ) - oEvent.getX( 1 ), 2 ) + Math.pow(
                            oEvent.getY( 0 ) - oEvent.getY( 1 ), 2 );
                    double dInitialDist = Math.pow( mClickPosX - mClickPosXZoom, 2 ) + Math.pow(
                            mClickPosY - mClickPosYZoom, 2 );
                    BattleView2D oBattleView = ( BattleView2D ) findViewById( R.id.battle );
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ( mClickPosXZoom == -1 && mClickPosYZoom == -1 )
                {
                    ...
                    double dDistanceSq = Math.pow( mClickPosX - fNewPosX, 2 ) + Math.pow(
                            mClickPosY - fNewPosY, 2 );
