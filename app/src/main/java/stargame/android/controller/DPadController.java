...
public class DPadController
{
	...
	private BattleAction GetElement( int iIndex, Tree< BattleAction > treeActions, int iExceptActionType )
	{
	...
	private String GetLeftActionString()
	{
		...
		{
			return GetElement( 1 + mActionMenuPos * 2, treeActions, R.string.move_action ).GetName( mContext );
		}
		else
		{
			return "";
	...
	private String GetRightActionString()
	{
		...
		{
			return GetElement( 2 + mActionMenuPos * 2, treeActions, R.string.move_action ).GetName( mContext );
		}
		else
		{
			return "";
	...
	private String GetBottomActionString()
	{
		...
		else
		{
			...
			if ( ( iNbMenuScreens == 1 && iNbActions < 4 ) || Orientation.modulo( iNbActions, 2 ) != 0 )
			{
				return "";
			}
			else
	...
	public void UpAction( Orientation oOrientation )
	{
		...
		case MENU_HIGH_LEVEL:
			...
			if ( !mBattle.GetActiveUnit().IsMovePerformed() )
			{
				mBattle.SetCurrentAction( mBattle.GetActiveUnit().GetAction( R.string.move_action ) );
		...
		case STATE_ACTION:
		case STATE_VIEW:
			mBattle.SetSelectedPosition( Position.OffsetPositionRelative( mBattle.GetSelectedPosition(), oOrientation, 0, -1 ) );
			break;
		}
		NotifyInterfaceDrawer();
		DisplayState();
	...
	public void DownAction( Orientation oOrientation )
	{
		...
		case MENU_ACTIONS:
			...
			{
				mBattle.SetCurrentAction( GetElement( iNbElements - 1, treeActions, R.string.move_action ) );
				mControlState = ControlState.STATE_ACTION;
			}
			break;
		case STATE_NONE:
	...
	public void LeftAction( Orientation oOrientation )
	{
		...
		case MENU_ACTIONS:
			...
			Tree< BattleAction > treeActions = mBattle.GetActiveUnit().GetActions();
			BattleAction oAction = GetElement( 1 + mActionMenuPos * 2, treeActions, R.string.move_action );
			if ( null != oAction )
			{
				mBattle.SetCurrentAction( oAction );
				mControlState = ControlState.STATE_ACTION;
		...
		case STATE_ACTION:
		case STATE_VIEW:
			mBattle.SetSelectedPosition( Position.OffsetPositionRelative( mBattle.GetSelectedPosition(), oOrientation, -1, 0 ) );
			break;
		}
		NotifyInterfaceDrawer();
		DisplayState();
	...
	public void RightAction( Orientation oOrientation )
	{
		...
		case MENU_ACTIONS:
			Tree< BattleAction > treeActions = mBattle.GetActiveUnit().GetActions();
			BattleAction oAction = GetElement( 2 + mActionMenuPos * 2, treeActions, R.string.move_action );
			if ( null != oAction )
			{
				mBattle.SetCurrentAction( oAction );
				mControlState = ControlState.STATE_ACTION;
