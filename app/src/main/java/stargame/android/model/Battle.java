...
public class Battle extends Observable implements ISavable
{
	...
	private Battle( XmlResourceParser oParser, Resources oResources )
	{
		...
		mVecUnits = new Vector< BattleUnit >();
		oUnit = new BattleUnit( mBattleField.GetCell( new Position( 1, 3 ) ), Orientation.EAST, JobType.TYPE_SOLDIER, this, oResources );
		oUnit.GetUnit().SetFaction( Faction.TYPE_MAGIK );
		mVecUnits.add( oUnit );
		oUnit = new BattleUnit( mBattleField.GetCell( new Position( 3, 3 ) ), Orientation.SOUTH, JobType.TYPE_MAGE, this, oResources );
		oUnit.GetUnit().SetFaction( Faction.TYPE_TECHNIK );
		mVecUnits.add( oUnit );
		oUnit = new BattleUnit( mBattleField.GetCell( new Position( 4, 3 ) ), Orientation.EAST, JobType.TYPE_PRIEST, this, oResources );
		oUnit.GetUnit().SetFaction( Faction.TYPE_MAGIK );
		mVecUnits.add( oUnit );
		oUnit = new BattleUnit( mBattleField.GetCell( new Position( 1, 1 ) ), Orientation.NORTH, JobType.TYPE_ROGUE, this, oResources );
		oUnit.GetUnit().SetFaction( Faction.TYPE_TECHNIK );
		mVecUnits.add( oUnit );
		oUnit = new BattleUnit( mBattleField.GetCell( new Position( 1, 4 ) ), Orientation.WEST, JobType.TYPE_BARBARIAN, this, oResources );
		oUnit.GetUnit().SetFaction( Faction.TYPE_TECHNIK );
		mVecUnits.add( oUnit );
		Init();
	...
	public void saveState( Bundle oObjectMap, Bundle oGlobalMap )
	{
		...
		for ( EndCondition oCondition : mVecEndConditions )
		{
			...
			oObjectMap.putString( String.format( "%s_%d", M_VEC_END_CONDITIONS_NAME, iIndex++ ), strObjKey );
		}
	...
	{
		Bundle oObjectBundle = SavableHelper.retrieveBundle( oGlobalMap, strObjKey, Battle.class.getName() );
