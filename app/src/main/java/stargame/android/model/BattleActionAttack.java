...
public class BattleActionAttack extends BattleAction implements ISavable
{
	private static class AttackStruct implements ISavable
	{
		...
		{
			Bundle oObjectBundle = SavableHelper.retrieveBundle( oGlobalMap, strObjKey, AttackStruct.class.getName() );
	...
	public void ComputeTargets()
	{
		...
		for ( int i = 0; i < iUnitRange; ++i )
		{
			...
			oNewPosition = oUnitPos.Offset( i + 1, 0 );
			if ( oNewPosition.mPosX < mBattle.GetBattleField().GetWidth() && IsAttackHeightPossible( oNewPosition ) )
			{
				mAttackArray[ oNewPosition.mPosX ][ oNewPosition.mPosY ].mAvailable = true;
				mVecTargetableCells.add( oNewPosition );
			}
			...
			oNewPosition = oUnitPos.Offset( 0, i + 1 );
			if ( oNewPosition.mPosY < mBattle.GetBattleField().GetHeight() && IsAttackHeightPossible( oNewPosition ) )
			{
				mAttackArray[ oNewPosition.mPosX ][ oNewPosition.mPosY ].mAvailable = true;
				mVecTargetableCells.add( oNewPosition );
			}
	...
	public void saveState( Bundle oObjectMap, Bundle oGlobalMap )
	{
		...
		Bundle oAttackBundle = SavableHelper.saveBidimensionalArrayInMap( mAttackArray, oGlobalMap );
	...
	protected void LoadBattleActionAttackState( Bundle oObjectMap, Bundle oGlobalMap )
	{
		...
		SavableHelper.loadBidimensionalArrayFromMap( mAttackArray, oArrayBundle, oGlobalMap, new AttackStruct() );
	}

	public static BattleActionAttack loadState( Bundle oGlobalMap, String strObjKey )
	{
		Bundle oObjectBundle = SavableHelper.retrieveBundle( oGlobalMap, strObjKey, BattleActionAttack.class.getName() );
