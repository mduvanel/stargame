...
public class BattleActionFactory implements ISavable
{
    ...
    public BattleAction loadState( Bundle oGlobalMap, String strObjKey )
    {
        ...
        // Base actions
        if ( ( oAction = BattleActionAttack.loadState( oGlobalMap, strObjKey ) ) != null )
        {
            return oAction;
        }
        if ( ( oAction = BattleActionMove.loadState( oGlobalMap, strObjKey ) ) != null )
        {
            return oAction;
        }

        // Priest actions
        if ( ( oAction = BattleActionBarrier.loadState( oGlobalMap, strObjKey ) ) != null )
        {
            return oAction;
        }
        if ( ( oAction = BattleActionHeal.loadState( oGlobalMap, strObjKey ) ) != null )
        {
            return oAction;
        }
