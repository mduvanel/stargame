...
public class BattleActionMove extends BattleAction implements ISavable
{
     ...
     */
    private void Move( BattleUnit oUnit, int iTargetPosX, int iTargetPosY, int iSourcePosX,
                       int iSourcePosY, double dCost, double dRemainingMvt )
    {
        // Si l'unité peut sauter la hauteur entre les deux cases
        if ( oUnit.GetUnit().GetResultingAttributes().GetVerticalJump() - mBattle.GetBattleField().GetElevationDiff(
                iSourcePosX, iSourcePosY, iTargetPosX, iTargetPosY ) >= 0 )
        {
            ...
            if ( mBattle.GetBattleField().IsPositionFree( iTargetPosX, iTargetPosY, oUnit ) )
            {
                ...
                if ( ( dRemainingMvt - dCost ) > oStruct.mRemainingMvt )
                {
                    ...
                    {
                        if ( mBattle.GetBattleField().GetCell( iTargetPosX,
                                                               iTargetPosY ).GetUnit() == null )
                        {
    ...
    private boolean CheckJumpable( int iSourcePosX, int iSourcePosY, int iTargetPosX,
                                   int iTargetPosY )
    {
        int iMaxHeight = Math.min(
                mBattle.GetBattleField().GetElevation( iSourcePosX, iSourcePosY ),
    ...
    private void StoreTrajectory()
    {
        ...
        Logger.d(
                String.format( "Destination position: %d, %d", oPosition.mPosX, oPosition.mPosY ) );
