            ...
            while ( eventType != XmlResourceParser.END_DOCUMENT )
            {
                if ( bLoadAnimations )
                {
                    for ( int iAnimation = 0; iAnimation < iAnimationsNb; ++iAnimation )
                    {
                        ...
                        XmlResourceParser oAnimationParser = oResources.getXml(
                                oResources.getIdentifier( strFilename, "xml",
                                                          "stargame.android" ) );

                        Logger.i(
                                String.format( "Loading Animation from file %s...", strFilename ) );
                ...
                else if ( eventType == XmlResourceParser.START_TAG )
                {
                    if ( oParser.getName().equals( "AnimationsNb" ) )
                    {
                        ...
                        mMapAnimationGroups = new HashMap< String, OrientableAnimation2D >(
                                iAnimationsNb );
                    }
    ...
    private void CheckIfCurrentAnimationIsFinished()
    {
        if ( !mNextAnimations.isEmpty() )
        {
            ...
            else
            {
                ...
                bFinished = mMapAnimationGroups.get(
                        mNextAnimations.getFirst().mName ).isFinished();
            }
            ...
            if ( bFinished )
            {
                ...
                if ( !mNextAnimations.isEmpty() && !mNextAnimations.getFirst().IsExternal() )
                {
                    ...
                    OrientableAnimation2D oGroup = mMapAnimationGroups.get(
                            mNextAnimations.getFirst().mName );
