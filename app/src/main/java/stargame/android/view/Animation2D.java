...
public class Animation2D extends Observable implements IDrawable
{
     ...
     * of Animation2DSteps.
     */
    public Animation2D( Vector< Bitmap > vecBitmaps, Vector< Animation2DStep > vecSteps,
                        boolean bLoop )
    {
            ...
            while ( eventType != XmlResourceParser.END_DOCUMENT )
            {
                if ( bLoadBitmaps )
                {
                    for ( int iBitmap = 0; iBitmap < iBitmapsNb; ++iBitmap )
                    {
                        ...
                        mBitmapResources[ iBitmapIndex ] = BitmapRepository.GetInstance().GetBitmap(
                                oParser.nextText() );
                ...
                else if ( bLoadFrames )
                {
                    for ( int iFrames = 0; iFrames < mNbAnimationSteps; ++iFrames )
                    {
                        ...
                        eventType = oParser.next();
                        mAnimationSteps[ iFrames ].mResourceIndex = Integer.parseInt(
                                oParser.getText() ) - 1;
    ...
    private void nextStep()
    {
        ...
        {
            DebugLogInfo( String.format( "nextStep called. Current step: %d, frame count: %d",
                                         mCurrentAnimationStep, mFrameCount ) );
            mFrameCount++;
        }
    ...
    public void doDraw( Canvas oCanvas, RectF oSourceRect, float fZoomFactor, Paint oPaint )
    {
        ...
        if ( IsAnimationRunning() )
        {
            ...
            if ( oCurrentStep.mResourceIndex > -1 ) // Resource index of -1 means do not display anything
            {
                ...
                mDrawingRect.offset( oCurrentStep.mXOffset * fZoomFactor,
                                     oCurrentStep.mYOffset * fZoomFactor );
                oCanvas.drawBitmap( mBitmapResources[ oCurrentStep.mResourceIndex ], null,
                                    mDrawingRect, oPaint );
            }
            else
            {
