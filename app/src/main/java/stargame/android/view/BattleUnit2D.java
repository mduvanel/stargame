...
public class BattleUnit2D implements Observer
{
	...
	public BattleUnit2D( BattleUnit oUnit, Context oContext, BattleDrawableList oDrawableList )
	{
		...
		mDrawable = new BattleDrawable( 
				...
				new RectF( 0, 0, DisplayConstants2D.GetCellWidth(), DisplayConstants2D.GetUnitHeight() ), 
				0 );
	...
	{
		String strName = String.format( "%s_%s_%s", mBattleUnit.GetUnit().GetJobType().toString(), eOrientation.toString().toLowerCase(), strType );
	...
	public void SetDrawingOrientation( Orientation eDrawingOrientation )
	{
		synchronized ( BattleThread2D.mDrawingLock )
		{
			...
			UpdateFromBattleCell();
			mAnimations.SetDrawingOrientation( mBattleUnit.GetOrientation().GetResultingOrientation( mDrawingOrientation ) );
		}
	}
	...
	public void SetBaseAnimation( String strAnimation )
	{
		synchronized ( BattleThread2D.mDrawingLock )
		{
			mAnimations.SetBaseAnimation( strAnimation, mBattleUnit.GetOrientation().GetResultingOrientation( mDrawingOrientation ) );
		}
	}
	...
	public void SetNextAnimation( String strAnimation, Orientation eAnimationOrientation )
	{
		synchronized ( BattleThread2D.mDrawingLock )
		{
			mAnimations.SetNextAnimation( strAnimation, eAnimationOrientation.GetResultingOrientation( mDrawingOrientation ) );
		}
	}

	public void SetNextAnimation( OrientableAnimation2D oAnimations, Orientation eAnimationOrientation )
	{
		synchronized ( BattleThread2D.mDrawingLock )
		{
			mAnimations.SetNextAnimation( oAnimations, eAnimationOrientation.GetResultingOrientation( mDrawingOrientation ) );
		}
	}
	...
	{
		int iCellZOrder = BattleField2D.GetInstance().GetCell2D( mBattleUnit.GetCell().GetPos() ).GetZOrder();
	...
	public void UpdateFromBattleCell()
	{
		...
		BattleField2D.GetInstance().FillCellAbsDrawingPosition( mBattleUnit.GetCell().GetPos(), mDrawingPosition );
	...
	public void update( Observable oObs, Object oObj )
	{
		...
		Logger.i( String.format( "BattleUnit2D receives change notification from BattleUnit. New pos is %s", mBattleUnit.GetCell().GetPos().toString() ) );
