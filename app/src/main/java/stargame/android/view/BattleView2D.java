...
public class BattleView2D extends SurfaceView implements SurfaceHolder.Callback
{
    ...
    public void UpdateCenterCellConstraints()
    {
    	...
    	case NORTH:
    	case SOUTH:
        	iRemainingX = Math.max( 0, BattleField2D.GetInstance().GetWidth() - mCellsDisplayedWidth );
        	iRemainingY = Math.max( 0, BattleField2D.GetInstance().GetHeight() - mCellsDisplayedHeight );
        	mMinCenterCellX = ( int )Math.floor( ( BattleField2D.GetInstance().GetWidth() - iRemainingX ) / ( double )2 ) ;
        	mMinCenterCellY = ( int )Math.floor( ( BattleField2D.GetInstance().GetHeight() - iRemainingY ) / ( double )2 ) ;
    		break;
    	case EAST:
    	case WEST:
        	iRemainingX = Math.max( 0, BattleField2D.GetInstance().GetHeight() - mCellsDisplayedWidth );
        	iRemainingY = Math.max( 0, BattleField2D.GetInstance().GetWidth() - mCellsDisplayedHeight );
        	mMinCenterCellX = ( int )Math.floor( ( BattleField2D.GetInstance().GetHeight() - iRemainingX ) / ( double )2 ) ;
        	mMinCenterCellY = ( int )Math.floor( ( BattleField2D.GetInstance().GetWidth() - iRemainingY ) / ( double )2 ) ;
    		break;
    	case NONE:
    		// Cannot happen
    		break;
	...
	public void CenterOn( Position oPos )
	{
    	...
    	case SOUTH:
    	case NORTH:
    		mCenterCellPos.mPosX = Math.max( mMinCenterCellX, Math.min( mMaxCenterCellX, oPos.mPosX ) );
        	mCenterCellPos.mPosY = Math.max( mMinCenterCellY, Math.min( mMaxCenterCellY, oPos.mPosY ) );
	    	break;
    	case EAST:
    	case WEST:
    		mCenterCellPos.mPosX = Math.max( mMinCenterCellY, Math.min( mMaxCenterCellY, oPos.mPosX ) );
        	mCenterCellPos.mPosY = Math.max( mMinCenterCellX, Math.min( mMaxCenterCellX, oPos.mPosY ) );
    		break;
    	case NONE:
    		// Should not happen
    		break;
	...
	public void SetZoomFactor( float fNewZoomFactor )
	{
		...
		mZoomFactor = Math.max( DisplayConstants2D.GetZoomMin(), Math.min( DisplayConstants2D.GetZoomMax(), iZoomStep * fStep ) );
