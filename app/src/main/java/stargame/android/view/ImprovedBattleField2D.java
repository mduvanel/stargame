...
/**
 * This is the ImprovedBattleField2D class, handling drawing of the BattleField in 2D
 * It is a singleton for convenience, there should be only one existing at a
 * time.
 */
public class ImprovedBattleField2D implements Observer
{
    /**
     ...
     */
    private static ImprovedBattleField2D msTheInstance = null;

    public static ImprovedBattleField2D GetInstance()
    {
        if ( msTheInstance == null )
        {
            Logger.w( "BattleField2D singleton created" );
            msTheInstance = new ImprovedBattleField2D();
        }
    ...
    public void Init( BattleField oBattleField, Resources oRes )
    {
        ...
        for ( int i = 0; i < iWidth; ++i )
        {
            ...
            {
                mArrayCells[ i ][ j ] = new BattleCell2D(
                        oBattleField.GetCell( new Position( i, j ) ), mDrawableList, -1 );
            }
        }
        ...
        mSelectedCellDrawable = new BattleDrawable( mSelectedCellAnimation, null, new RectF( 0, 0,
                                                                                             DisplayConstants2D.GetCellWidth(),
    ...
    private void LoadBitmaps( Resources oRes )
    {
        ...
        mTileSets.put( FieldType.TYPE_WATER,
                       new TileSet2D( oRes.getXml( R.xml.tileset_water ), oRes ) );
    ...
    public void FillCellAbsDrawingPosition( Position oCellPos, Position oDrawingPos )
    {
        ...
        oDrawingPos.mPosY -= DisplayConstants2D.GetCellStackHeight() * mBattleField.GetCell(
                oCellPos ).GetElevation();
    }
    ...
    */
    private boolean IsVerticalTilePlain( int iVerticalTileHeight, BattleCell oCell,
                                         BattleCell oSideCell, BattleCell oLowerSideCell )
    {
        return ( oSideCell != null && oSideCell.GetType() == oCell.GetType() && oSideCell.GetElevation() >= iVerticalTileHeight );
    }

    private boolean IsPlainTileAngle( BattleCell oCell, BattleCell oSideCell,
                                      BattleCell oVerticalCell, BattleCell oCornerCell )
    {
        return ( oSideCell != null && oVerticalCell != null && oCornerCell != null &&
                ( ( oVerticalCell.GetType() == oCell.GetType() && oVerticalCell.GetElevation() == oCell.GetElevation() && IsTileDivision(
                        oVerticalCell, oCornerCell ) ) ||
                        ( oSideCell.GetType() == oCell.GetType() && oSideCell.GetElevation() == oCell.GetElevation() && IsTileDivision(
                                oSideCell, oCornerCell ) ) ) );
    }
        ...
        for ( oPos.mPosY = 0; oPos.mPosY < iHeight; ++oPos.mPosY )
        {
            for ( oPos.mPosX = 0; oPos.mPosX < iWidth; ++oPos.mPosX )
            {
                ...
                if ( oCellBelow == null || oCellBelow.GetElevation() < oCell.GetElevation() )
                {
                    ...
                    {
                        // Top part, left
                        if ( IsVerticalTilePlain( oCell.GetElevation(), oCell, oCellLeft,
                                                  oCellBelowLeft ) )
                        {
                            eType = TileType.TOP;
                        }
                        ...
                        oDrawable = mVerticalTileSets.get( oCell.GetType() ).GetDrawable( eType,
                                                                                          TilePos.UPPER_LEFT );
                        oDestRect.offset( -iCellWidth / 2, iCellHeight / 2 );
                        mArrayCells[ oPos.mPosX ][ oPos.mPosY ].AddTile( oDrawable, oDestRect,
                                                                         true );

                        // Top part, right
                        if ( IsVerticalTilePlain( oCell.GetElevation(), oCell, oCellRight,
                                                  oCellBelowRight ) )
                        {
                            eType = TileType.TOP;
                        }
                        ...
                        while ( iDiff > 1 )
                        {
                            if ( IsVerticalTilePlain( oCell.GetElevation() - ( iDiffOrig - iDiff ),
                                                      oCell, oCellLeft, oCellBelowLeft ) )
                            {
                                eType = TileType.PLAIN;
                            }
                            else
                            ...
                            oDrawable = mVerticalTileSets.get( oCell.GetType() ).GetDrawable( eType,
                                                                                              ( bUpper ? TilePos.UPPER_LEFT : TilePos.LOWER_LEFT ) );
                            oDestRect.offset( -iCellWidth / 2, iStackHeight );
                            mArrayCells[ oPos.mPosX ][ oPos.mPosY ].AddTile( oDrawable, oDestRect,
                                                                             true );

                            if ( IsVerticalTilePlain( oCell.GetElevation() - ( iDiffOrig - iDiff ),
                                                      oCell, oCellRight, oCellBelowRight ) )
                            {
                                eType = TileType.PLAIN;
                            }
                            else
                            ...
                            oDrawable = mVerticalTileSets.get( oCell.GetType() ).GetDrawable( eType,
                                                                                              ( bUpper ? TilePos.UPPER_RIGHT : TilePos.LOWER_RIGHT ) );
                            oDestRect.offset( iCellWidth / 2, 0 );
                            mArrayCells[ oPos.mPosX ][ oPos.mPosY ].AddTile( oDrawable, oDestRect,
                                                                             true );
                    ...
                    if ( oCellBelow != null )
                    {
                        ...
                        oBackgroundDrawable = mTileSets.get( oCellBelow.GetType() ).GetDrawable(
                                eType, TilePos.LOWER_LEFT );
                        mArrayCells[ oPos.mPosX ][ oPos.mPosY ].AddTile( oBackgroundDrawable,
                                                                         oDestRect, false );
                    }

                    if ( IsVerticalTilePlain( oCell.GetElevation() - ( iDiffOrig - iDiff ), oCell,
                                              oCellLeft, oCellBelowLeft ) )
                    {
                        if ( bMiddle )
                        {
                            eType = TileType.PLAIN;
                    ...
                    if ( oCellBelow != null )
                    {
                        ...
                        oBackgroundDrawable = mTileSets.get( oCellBelow.GetType() ).GetDrawable(
                                eType, TilePos.LOWER_RIGHT );
                        mArrayCells[ oPos.mPosX ][ oPos.mPosY ].AddTile( oBackgroundDrawable,
                                                                         oDestRect, false );
                    }

                    if ( IsVerticalTilePlain( oCell.GetElevation() - ( iDiffOrig - iDiff ), oCell,
                                              oCellRight, oCellBelowRight ) )
                    {
                        if ( bMiddle )
                        {
                            eType = TileType.PLAIN;
    ...
    {
        return mBattleField.GetCell(
                Position.OffsetPositionRelative( oPos, mDrawingOrientation, iXOffsetWest,
                                                 iYOffsetNorth ) );
    }
    ...
    public void UpdateSelectedCellDrawable()
    {
        ...
        // Update ZOrder
        if ( mSelectedCellDrawable.SetZOrder( GetCell2D(
                mBattleField.GetSelectedCell().GetPos() ).GetZOrder() + ZOrderPosition.SELECTED_ANIM.Value() ) )
        {
