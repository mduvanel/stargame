...
import stargame.android.view.Animation2DStep;
...
public class MoveAnimation2DFactory
{
			...
			Observer oObserver )
	{
		...
		int iDistance = Math.abs( ( oSourcePos.mPosX + oSourcePos.mPosY ) - ( oDestPos.mPosX + oDestPos.mPosY ) );
		...
		if ( bAnimate )
		{
			...
			for ( int i = 0; i < Orientation.NONE.ordinal(); ++i )
			{
				...
				int iOffset = GetDestZOffset( oSourcePos, oDestPos, BattleField2D.GetInstance().GetDrawingOrientation() );
	...
	public static int GetDestZOffset( Position oSourcePos, Position oDestPos, Orientation eDrawingOrientation )
	{
		int iDiff = 0;
