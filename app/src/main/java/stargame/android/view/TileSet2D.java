...
public class TileSet2D
{
	...
	private static int TilesetBitmapIdFromParams( TileType eType, TilePos ePos, String strName, int iStep, Resources oRes )
	{
		String strFullname = String.format( "%s_%s_%s", strName, ePos.toString(), eType.toString() );
		if ( iStep != -1 )
		{
		...
		for ( TilePos ePos : TilePos.values() )
		{
			...
			for ( TileType eType : TileType.values() )
			{
				...
				if ( index > 0 )
				{
					...
					{
						Logger.e( String.format( "Error loading bitmap %s, %s of tileset %s, resource missing!", eType.toString(), ePos.toString(), mSetName ) );
					}
				}
			}
			...
			for ( TileType eType : TileType.values() )
			{
				try
				{
					...
					else
					{
						...
						{
							int index = TilesetBitmapIdFromParams( eType, ePos, mSetName, i + 1, oResources );
							if ( index > 0 )
							{
