...
public class BattleActionTarget2D
{
	...
	public void UpdateDrawables( BattleAction oAction )
	{
		...
		synchronized ( BattleThread2D.mDrawingLock )
		{
			...
			if ( mBattleAction != null )
			{
				mAnimations.SetBaseAnimation( GetAnimationStringFromAction( mBattleAction ), Orientation.NONE );
				...
				{
					RectF oDestRect = new RectF( 0, 0, DisplayConstants2D.GetCellWidth(), DisplayConstants2D.GetCellHeight() );
					...
					int iZOrder = BattleField2D.GetInstance().GetCell2D( oPos ).GetZOrder() + ZOrderPosition.TARGET_ANIM.Value();
					BattleDrawable oDrawable = new BattleDrawable( mAnimations.GetCurrentAnimation().Clone(), null, oDestRect, iZOrder );
